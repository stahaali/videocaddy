/**
 * Scrape Video Caddy animation pages into JSON + local images.
 * Run: node scripts/scrape-animation.mjs
 */
...
const outDir = path.join(root, "src", "data", "animation", "pages");
const imgRoot = path.join(root, "public", "assets", "images", "animation");

const FALLBACK_BG = "/assets/images/cartoon-storyboard-bg-banner.webp";

const PAGES = [
  { slug: "index", url: "https://www.videocaddy.com/animation/", label: "Animation Services" },
  { slug: "3d-engineering", url: "https://www.videocaddy.com/animation/3d-engineering-animation.php", label: "3D Engineering Animation" },
  { slug: "animated-product-advertisement", url: "https://www.videocaddy.com/animation/animated-product-advertisement.php", label: "Animated Product Advertisement" },
  { slug: "2d-animation", url: "https://www.videocaddy.com/animation/2d-animation.php", label: "2D Animation" },
  { slug: "3d-animation", url: "https://www.videocaddy.com/animation/3d-animation.php", label: "3D Animation" },
  { slug: "3d-product-animation", url: "https://www.videocaddy.com/animation/3d-product-animation.php", label: "3D Product Animation" },
  { slug: "architecture", url: "https://www.videocaddy.com/animation/architectural.php", label: "Architecture Animation" },
  { slug: "character", url: "https://www.videocaddy.com/animation/character.php", label: "Character Animation" },
  { slug: "cutout", url: "https://www.videocaddy.com/animation/cutout-animation.php", label: "Cutout Animation" },
  { slug: "explainer-video", url: "https://www.videocaddy.com/animation/explainer-video.php", label: "Explainer Video Animation" },
  { slug: "feature-film", url: "https://www.videocaddy.com/animation/feature-film.php", label: "Feature Film Animation" },
  { slug: "flash", url: "https://www.videocaddy.com/animation/flash.php", label: "Flash Animation" },
  { slug: "logo", url: "https://www.videocaddy.com/animation/logo.php", label: "Logo Animation" },
  { slug: "machine-assembly", url: "https://www.videocaddy.com/animation/machine-assembly-animation.php", label: "Machine Assembly Animation" },
  { slug: "product", url: "https://www.videocaddy.com/animation/product.php", label: "Product Animation" },
  { slug: "rotoscoping", url: "https://www.videocaddy.com/animation/rotoscoping.php", label: "Rotoscoping Animation" },
  { slug: "titling", url: "https://www.videocaddy.com/animation/titling.php", label: "Titling Animation" },
  { slug: "whiteboard", url: "https://www.videocaddy.com/animation/white-board.php", label: "Whiteboard Animation" },
  { slug: "stop-motion", url: "https://www.videocaddy.com/animation/stop-motion-animation.php", label: "Stop Motion Animation" },
  { slug: "tv-commercial", url: "https://www.videocaddy.com/animation/tv-commercial.php", label: "TV Commercial Animation" },
  { slug: "medical", url: "https://www.videocaddy.com/animation/medical-animation.php", label: "Medical Animation" },
];
...
function extractServicesHeader(html) {
  const titleMatch = html.match(
    /<h2[^>]*>\s*((?:<[^>]+>\s*)*(?:Our[\s\S]{0,120}?Services|OUTSOURCE YOUR|STUNNING[\s\S]{0,40}|ADOBE[\s\S]{0,40}|PROFESSIONAL[\s\S]{0,80}Services|Storyboard[\s\S]{0,60}Services|Animation[\s\S]{0,60}Services)[\s\S]*?)<\/h2>/i
  );
...
function extractBannerBackground(html) {
  ...
  const pageSpecific = unique.find((u) =>
    /storyboard-bg|cartoon-storyboard-bg|animation-bg|character-|comic-|e-?learning|illustration|photomatic|animatic|video-game|product-anim|3d-|2d-|whiteboard|logo|medical|feature-film/i.test(
      u
    )
  );
...
async function scrapePage(page) {
  ...
  const heroLocal = `/assets/images/animation/hero/${page.slug}.${heroExt}`;
  ...
  if (bgUrl) {
    ...
    if (/cartoon-storyboard-bg-banner/i.test(bgUrl) && fallbackExists) {
      ...
      const bgLocal = `/assets/images/animation/bg/${bgName}.${bgExt}`;
  ...
  for (let i = 0; i < rawServices.length; i++) {
    ...
    if (s.imageUrl) {
      ...
      image = `/assets/images/animation/services/${page.slug}-${i}.${ext}`;
  ...
  if (whyImgUrl) {
    ...
    whyImage = `/assets/images/animation/why/${page.slug}.${ext}`;
